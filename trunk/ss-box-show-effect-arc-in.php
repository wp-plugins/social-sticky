...
<div class=" <?=$ss_div_effect_class?>" style="width:<?=$ss_svg?>px; height:<?=$ss_svg?>px" >
	<svg width="<?=$ss_svg?>" height="<?=$ss_svg?>">
		<defs>
			<clipPath id="ss-imgArcIn-clipPath">
			  <rect x="<?=$ss_logo_x-$ss_logo_w/2?>" y="<?=$ss_logo_y-$ss_logo_h/2?>" rx="4" ry="4" width="<?=$ss_logo_w?>" height="<?=$ss_logo_h?>"></rect>
			</clipPath>
			<linearGradient id="ss_grad_ArcIn" x1="<?=$ss_grad_EffectArcIn_x1?>" y1="<?=$ss_grad_EffectArcIn_y1?>" x2="<?=$ss_grad_EffectArcIn_x2?>" y2="<?=$ss_grad_EffectArcIn_y2?>">
		...
		<path id="<?=$ss_arc_id?>" d="<?=$ss_arcIn_path_d?>" style="stroke: #c3c3c3; stroke-width:1;" fill="url(#ss_grad_ArcIn)"/>

		<g id=<?=$ss_animation_id?> transform="rotate(<?=-$ss_logo_rotate_value?> <?=$ss_arc_cx?> <?=$ss_arc_cy?>)">
			<image id="<?=$ss_logo_id?>" x="<?=$ss_logo_x-$ss_logo_w/2?>" y="<?=$ss_logo_y-$ss_logo_h/2?>" width="<?=$ss_logo_w?>" height="<?=$ss_logo_h?>" xlink:href="<?=$img_url?>" style="clip-path: url(#ss-imgArcIn-clipPath);" transform="rotate(<?=$ss_logo_rotate_value?> <?=$ss_logo_x?> <?=$ss_logo_y?>)"/>
		</g>
	</svg>
</div>
